...
public partial class SettingsUserControl : UserControl
{
    ...
    private void AddSampleDevices()
    {
        ...
        DeviceList.Add(new DeviceItem
        {
            ...
            Status = "有新版本",
            ButtonBackground = updateGradient,
            UpdateCommand = new RelayCommand(param => ShowUpdateDialog(0)),
            UpdateDescription = "本次更新包含重要优化与问题修复\n提升系统稳定性\n优化响应速度\n修复已知问题"
        });
        ...
        DeviceList.Add(new DeviceItem
        {
            ...
            UpdateCommand = new RelayCommand(param => ShowUpdateDialog(1)),
            UpdateDescription = ""
        });
    }

    private void ShowUpdateDialog(int deviceIndex)
    {
        if (deviceIndex < 0 || deviceIndex >= DeviceList.Count) return;

        var device = DeviceList[deviceIndex];

        if (device.Status != "有新版本") return;

        var parentWindow = Window.GetWindow(this);
        if (parentWindow is MainWindow mainWindow)
        {
            var dialog = mainWindow.GlobalDialog;
            dialog.Title = $"{device.Model} 更新提示";
            dialog.ClearButtons();
            ...
            dialog.DialogContent = descText;

            dialog.AddButton("立即更新", (s, e) =>
            {
                dialog.Hide();
                StartDeviceUpdate(deviceIndex);
            }, true);
    ...
    private async void StartDeviceUpdate(int deviceIndex)
    {
        if (deviceIndex < 0 || deviceIndex >= DeviceList.Count) return;
        ...
        if (parentWindow is MainWindow mainWindow)
        {
            ...
            // === 修复 1：重置弹窗的基础状态，防止上一个界面的标题和图标残留 ===
            ...
            titleShadow.Foreground = shadowGradient;
            Canvas.SetLeft(titleShadow, 0);
            Canvas.SetTop(titleShadow, 0);
            ...
            var percentBorder = new Border
            {
                ...
                BorderThickness = new Thickness(0, 0, 0, 3), // 底部边框替代原来的 percentLine
                Margin = new Thickness(0, 0, 0, 4),
                Padding = new Thickness(0, 0, 0, 0), // 文字与底边框的间距
                Width = 125,
                Height = 36,
                HorizontalAlignment = HorizontalAlignment.Left
            };
            ...
            // ================== 第二行：进度条中控区域 ==================
            var middleSectionGrid = new Grid
            {
                VerticalAlignment = VerticalAlignment.Center,
                Margin = new Thickness(0, 0, 0, 0)
            };
            middleSectionGrid.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto }); // 上部：左上角、文字、右上角
            middleSectionGrid.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto }); // 中部：进度条
            middleSectionGrid.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto }); // 下部：左下角、文字、右下角
            ...
            var updatingText = new TextBlock
            {
                Text = "UPDATING...",
                FontSize = 12,
                Foreground = new SolidColorBrush(Color.FromRgb(198, 14, 14)),
                FontWeight = FontWeights.Bold,
                FontFamily = new FontFamily(new Uri("pack://application:,,,/"), "./Assets/Fonts/#Orbitron"),
                HorizontalAlignment = HorizontalAlignment.Left,
                VerticalAlignment = VerticalAlignment.Bottom,
            ...
            var progressBarContainer = new Grid
            {
                ...
                Height = 12,
                Margin = new Thickness(0, 0, 0, 0),
                Background = Brushes.Transparent // 确保背景透明
            };
            ...
            greenBrush.GradientStops.Add(new GradientStop(Color.FromRgb(17, 98, 47), 0));   // #11622F
            greenBrush.GradientStops.Add(new GradientStop(Color.FromRgb(76, 224, 131), 1)); // #4CE083
            ...
            redBrush.GradientStops.Add(new GradientStop(Color.FromRgb(198, 14, 14), 0));    // #C60E0E
            redBrush.GradientStops.Add(new GradientStop(Color.FromRgb(96, 7, 7), 1));      // #600707
            ...
            var pleaseWaitLine = new Border
            {
                ...
                HorizontalAlignment = HorizontalAlignment.Left,
                Margin = new Thickness(4.3, 0, 0, 0)
            };
            ...
            for (int i = 0; i <= 100; i += 2)
            {
                ...
                percentText.Text = $"{i}%";
                colGreen.Width = new GridLength(i, GridUnitType.Star);
                colRed.Width = new GridLength(100 - i, GridUnitType.Star);
            }
    ...
    private void ViewUpdateLogButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (parentWindow is MainWindow mainWindow)
        {
            ...
            // 关键修改：调用 ClearButtons 确保之前的按钮被清除，但不添加新按钮
            dialog.ClearButtons();

            // 移除原有的 dialog.AddButton("关闭", ...) 代码

            var scrollViewer = new ScrollViewer { MaxHeight = 300, VerticalScrollBarVisibility = ScrollBarVisibility.Auto, HorizontalAlignment = HorizontalAlignment.Stretch, Margin = new Thickness(0, 17, 0, -5)};
            var stackPanel = new StackPanel { HorizontalAlignment = HorizontalAlignment.Stretch };

            // 根据图片内容重新编写文本结构
            stackPanel.Children.Add(new TextBlock
            {
                Text = "版本 v1.1.1 发布于2026-04-11",
                Foreground = new SolidColorBrush(Color.FromRgb(238, 238, 238)),
                FontSize = 22,
                FontWeight = FontWeights.Regular,
                HorizontalAlignment = HorizontalAlignment.Left,
